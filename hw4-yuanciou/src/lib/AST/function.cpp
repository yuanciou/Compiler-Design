...
std::string FunctionNode::getParameterString() const
{
    return getParametersTypeString(m_parameters);
}

std::vector<PType*> FunctionNode::getParametersType()
{
    std::vector<PType*> parameters;
    for(auto &parameter : m_parameters)
    {
        for (const auto &variable : parameter->getVariables())
        {
            parameters.push_back(variable->getType());
        }
    }
    return parameters;
}
